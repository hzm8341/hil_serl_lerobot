...
def _select_transition_action(info, action):
    if info.get("is_intervention") and "action_intervention" in info:
        return info["action_intervention"], True
    return action, False
...
def act_with_policy(
    ...
    for interaction_step in range(cfg.policy.online_steps):
        ...
        # Override the action only when the environment provides a concrete intervention action.
        action, did_intervene = _select_transition_action(info=info, action=action)
        if did_intervene:
