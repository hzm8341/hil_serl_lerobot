...
def _select_recorded_action(info, policy, action):
    if policy is not None:
        return action

    recorded_action = info.get("action_intervention", action)
...
def record_dataset(env, policy, cfg):
    ...
    while episode_index < cfg.num_episodes:
        ...
        while time.perf_counter() - start_episode_t < cfg.wrapper.control_time_s:
            ...
            recorded_action = {
                "action": _select_recorded_action(info=info, policy=policy, action=action)
            }
